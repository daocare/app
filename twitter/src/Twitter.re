...
Js.Re.
          let optMatch = Js.Re.exec(tweet.text, emojiRegex);
          Js.log(tweet.text);

          switch (optMatch) {
          | Some(match) =>
            let results = match->Js.Re.captures;
            Js.log2("Length - ", results->Array.length);
            let optFirstMatch = results[0] >>= Js.Nullable.toOption;
            switch (optFirstMatch) {
            | Some(match) =>
              let isSecondMatch =
                (results[1] >>= Js.Nullable.toOption)
                ->Option.mapWithDefault(false, _ => true);
              Js.log2(results[1], "<<-second match");
              if (isSecondMatch) {
                Js.log("there were 2 emojis!");
              } else {
                Js.log("perfect match");
                // Js.log(match);
                ();
              };
            | None => Js.log("no match!")
            };
          // let newArtworkId = results[3];
...
let start = () => {
  ...
  Js.Global.setInterval(loopFunction, 20000)->ignore;
  ()->async;
};
